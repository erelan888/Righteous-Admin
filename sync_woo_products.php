<?php
    ...
    function upload_product_variation($_product_id, $_variation_id, $client_id, $client_url){
        ...
        . $result_array->manage_stock . "','"
        . $result_array->stock_quantity . "','"
    ...
    if(isset($_GET['client_id'])){
        ...
        $request_url = $client_url . "/wp-json/wc/v3/products?" . "consumer_key=" . $consumer_key . "&consumer_secret=" . $consumer_secret . "&per_page=100&page=1" ;
...
//Have data...do thing
        foreach($result_array as $product){
            ...
            foreach($product->variations as $variation){
                //Upload variation here
                if(check_if_variation_exists($client_id,$variation) < 1){
                    upload_product_variation($product->id, $variation, $client_id, $client_url);
                }
                sleep(1);
            }
        }
        ...
        while($row = mysqli_fetch_assoc($cred_results)){
            ...
            $client_url = get_client_url($client_id);
            $request_url = $client_url . "/wp-json/wc/v2/orders?" . "consumer_key=" . $consumer_key . "&consumer_secret=" . $consumer_secret;
            //get client url /wp-json/wc/v2/orders
            $ch = curl_init($request_url);
