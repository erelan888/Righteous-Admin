<?php
    ...
    if(isset($_GET['design_id'])){
        ...
        $update_query = "UPDATE `admin_heatpress_design_list` SET " . $column_to_update . "=\"" . $data . "\" WHERE _id=" . $design_id;

        if($conn->query($update_query) === TRUE){
            $response .="Updated";
            //add record to product change time series
            $tracking_query = "INSERT INTO `admin_heatpress_changes` (heatpress_id, username, design_changes) VALUES (\"" 
